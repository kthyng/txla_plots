'''
Make sea surface height plots for movies of the full domain.
...
for year in years:
    ...
    for plotdate in plotdates:
        ...
        figname = 'figures/ssh/movies/' + pd.to_datetime(plotdate.data).isoformat()[0:13] + '.png'
        ...
        var = m.zeta.sel(ocean_time=plotdate).isel(eta_rho=slice(1,-1), xi_rho=slice(1,-1))
        ...
        qk = ax.quiverkey(Q, 0.12, 0.79, 0.5, r'0.5 m$\cdot$s$^{-1}$ current', labelcolor='0.2', fontproperties={'size': '10'})
        ...
        qk = ax.quiverkey(Q, 0.12, 0.84, 10, r'10 m$\cdot$s$^{-1}$ wind', labelcolor='0.2', fontproperties={'size': '10'})

        # Colorbar in upper left corner
        cax = fig.add_axes([0.085, 0.92, 0.32, 0.018]) #colorbar axes
        cb = fig.colorbar(mappable, cax=cax, orientation='horizontal')
        cb.set_label(r'Sea surface height [m]            ', fontsize=13, color='0.2')
