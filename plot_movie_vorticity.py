'''
Make vorticity plots for movies of the full domain.
...
for year in years:
    ...
    for plotdate in plotdates:
        ...
        # Plot variable
        u = m.u.sel(ocean_time=plotdate).isel(s_rho=-1).data
        v = m.v.sel(ocean_time=plotdate).isel(s_rho=-1).data
        # Vertical vorticity, and assuming that vectors don't need to be rotated before calculating vorticity.
        var = (v[:, 1:] - v[:, :-1])*pm - (u[1:, :] - u[:-1, :])*pn
        var /= f
        ...
        qk = ax.quiverkey(Q, 0.10, 0.755, 0.5, r'0.5 m$\cdot$s$^{-1}$ current', labelcolor='0.2', fontproperties={'size': '10'})
        ...
        qk = ax.quiverkey(Q, 0.10, 0.82, 10, r'10 m$\cdot$s$^{-1}$ wind', labelcolor='0.2', fontproperties={'size': '10'})
        ...
        cb.set_label('Normalized surface vertical            \nvorticity ' + r'$[\zeta/f]$                            ', fontsize=13, color='0.2')
